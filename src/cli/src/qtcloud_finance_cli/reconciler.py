        ...
        for entry in entries:
            if isinstance(entry, data.Transaction):
                ...
                for post in entry.postings:
                    if post.account.startswith("Assets:") or post.account.startswith(
                        "Liabilities:"
                    ):
...
def score_match(bank: Transaction, ledger: Transaction) -> float:
    ...
    date_diff = (
        abs((bank_date - ledger_date).days) if bank_date and ledger_date else 999
    )
    amount_diff = abs(bank.amount - ledger.amount)
...
def match_transactions(
    bank: List[Transaction], ledger: List[Transaction], settings: Settings
...
def build_prompt(
    bank_unmatched: List[Transaction], ledger_unmatched: List[Transaction]
) -> tuple[str, str]:
...
def reconcile(
    bank_file: str, ledger_file: Optional[str] = None
    ...
    ledger_path = (
        Path(ledger_file) if ledger_file else settings.data_root / "main.beancount"
    )
    ...
    logger.info(
        "自动匹配完成: %d 对, 未匹配: bank=%d, ledger=%d",
        len(auto_pairs),
        len(unmatched_bank),
